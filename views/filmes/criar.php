...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!Csrf::validar($_POST['csrf_token'] ?? null)) {
        ...
        // Chama o método do controller para criar o filme
        ...
        if ($resultado['ok']) {
            header('Location: /projeto/views/filmes/listar.php?msg=criado');
            exit;
        }
    ...
    <form method="POST" action="">
        ...
        <label>Descricao
            <textarea name="descricao" rows="5" 
                      maxlength="5000"><?= htmlspecialchars($_POST['descricao'] ?? '') ?></textarea>
        </label>
